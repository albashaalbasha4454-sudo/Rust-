...
const POSView: React.FC<POSViewProps> = ({ products, modifiers, customers, departments, onCompleteSale, onCreateDeliveryOrder, onCreateReservation }) => {
  ...
  const categories = useMemo(() => {
    const relevantProducts = selectedDeptId === 'all' 
        ? products 
        : products.filter(p => p.departmentId === selectedDeptId);
    const cats = new Set(relevantProducts.map(p => p.category || 'غير مصنف'));
    return ['الكل', ...Array.from(cats)];
  }, [products, selectedDeptId]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [products, searchTerm, selectedCategory, selectedDeptId]);
  ...
  const addToCart = (product: Product) => {
    ...
    setCart([...cart, {
      ...
      productName: product.name,
      departmentId: product.departmentId || 'misc',
      departmentName: product.departmentName || 'عام',
      quantity: 1,
      basePrice: basePrice,
...
const confirmAddToCart = () => {
    ...
    setCart([...cart, {
        ...
        departmentId: addingProduct.departmentId || 'misc',
        departmentName: addingProduct.departmentName || 'عام',
        quantity: 1,
        basePrice: basePrice,
        costPrice: addingProduct.costPrice,
        ...
        <div className="bg-white border-b border-slate-200 p-4 sm:p-6 shadow-sm z-20">
          ...
          <div className="mt-6 flex flex-col gap-4">
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar scroll-smooth">
              <button
                onClick={() => { setSelectedDeptId('all'); setSelectedCategory('الكل'); }}
                className={`px-6 py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-300 border ${selectedDeptId === 'all' ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100 transform -translate-y-0.5' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300 hover:text-indigo-600'}`}
              >
                الكل
              </button>
              {departments.map(dept => (
                <button
                  key={dept.id}
                  onClick={() => { setSelectedDeptId(dept.id); setSelectedCategory('الكل'); }}
                  className={`px-6 py-2 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-300 border ${selectedDeptId === dept.id ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100 transform -translate-y-0.5' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300 hover:text-indigo-600'}`}
                >
                  {dept.name}
                </button>
              ))}
            ...
            {selectedDeptId !== 'all' && categories.length > 1 && (
              <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar scroll-smooth">
                {categories.map(cat => (
                  <button
                    key={cat}
                    onClick={() => setSelectedCategory(cat)}
                    className={`px-4 py-1.5 rounded-lg text-[10px] sm:text-xs font-bold whitespace-nowrap transition-all duration-300 border ${selectedCategory === cat ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-200'}`}
                  >
                    {cat}
                  </button>
                ))}
                        ...
                        <div className="space-y-2">
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              <div className="flex-1">
                                  <label className="text-[8px] font-bold text-slate-400 block">سعر البيع</label>
                                  <input 
                                      type="number" 
                                      placeholder="البيع" 
                                      value={item.basePrice || ''} 
                                      onChange={e => updateCartItem(index, { basePrice: parseFloat(e.target.value) || 0 })}
                                      className="w-full text-xs text-indigo-600 font-black bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5 outline-none focus:bg-white focus:border-indigo-400 transition-all font-mono"
                                  />
                              </div>
                              <div className="flex-1">
                                  <label className="text-[8px] font-bold text-slate-400 block">التكلفة (للربح)</label>
                                  <input 
                                      type="number" 
                                      placeholder="التكلفة" 
                                      value={item.costPrice || ''} 
                                      onChange={e => updateCartItem(index, { costPrice: parseFloat(e.target.value) || 0 })}
                                      className="w-full text-xs text-slate-600 font-black bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5 outline-none focus:bg-white focus:border-slate-400 transition-all font-mono"
                                  />
                              </div>
                           </div>
                        </div>
                       ...
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div className="relative">
                             ...
                             <select 
                               ...
                               onChange={e => {
                                 const selectedDept = departments.find((d: Department) => d.id === e.target.value);
